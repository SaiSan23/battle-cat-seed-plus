...
function renderToggles(entries) {
  ...
  for (const e of entries) {
    ...
    // 逐卡池強制保證（放 label 外，避免點擊誤觸顯示開關）
    const sel = document.createElement('select');
    sel.className = 'gu-sel';
    sel.title = '保證顯示：原生＝該場次自帶的必中（godfat 自動判定）；強制＝以 force_guaranteed 模擬';
    sel.innerHTML =
      '<option value="">原生</option>' +
      GU_SIZES.map((s) => `<option value="${s}">強制${s}連</option>`).join('');
...
function renderTargetChips() {
  ...
  document.querySelector('#route-hint').hidden = routeTargets.size > 0; // 提示只在尚無目標時顯示
}
...
function showPlanPath(plan) {
  ...
  document.querySelector('#path-legend').hidden = !plan; // 圖例跟著路徑高亮同進退
  if (!plan) return;
  ...
  requestAnimationFrame(() => setTimeout(() => {
    try {
      ...
      const banners = currentEntries.map((e) => {
        ...
        // GU 尺寸：強制值優先，否則由解析結果推導原生必中尺寸（無保證格 → null，不排 GU）
        return { id: e.banner.id, short: e.banner.short, start, end,
          hidden: hidden.has(e.banner.id),
          gu: Number(guFor(e.banner.id)) || guaranteedSize(e.parsed.cells) };
      });
      const merged = mergeBanners(currentEntries);
...
(() => {
  ...
  // 窄視窗改走 CSS 貼邊版面：不套記憶座標、不啟動拖曳（inline style 會蓋掉 media query）
  ...
  try {
    ...
    if (!narrow() && saved && Number.isFinite(saved.left) && Number.isFinite(saved.top)) apply(saved.left, saved.top);
  } catch { /* 壞值 → 用預設位置 */ }
