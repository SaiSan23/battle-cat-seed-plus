...
## cats-tw-owned.html（2026-07-09 新增）

godfat `/cats?lang=tw&o=4ZIEPP2m` 全貓清單頁，帶 `o=` 持有短碼（貓 1/2/44 已勾），供
`lib/owned.js` 解析「已擁有貓 id」。

- 每隻貓一個 `<input type="checkbox" name="t" value="<id>">`；帶 `o=` 開頁時已擁有者
  多一個 `checked="checked"` 屬性（未擁有者無此屬性）。
- `?t=<id>…` 帶多個 `t` 參數送出後，godfat 302 轉址，新的 `o=` 短碼就在轉址後網址上
  （非回應內容）——換碼需讀 `res.url`，不能只讀回應本文。
- 種子頁（抽卡表）帶 `o=` 時，已擁有貓的格子會多套 `owned` class，純顯示用，
  不影響抽卡計算與稀有度判定（見前述「防禦性註記：find / o 參數會改格子 class」）。
